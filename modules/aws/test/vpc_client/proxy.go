...
func (vpc *VPC) LaunchProxyInstance(imageID string, zone string, sshKey string) (types.Instance, string, string, error) {
	...
	instOut, err := vpc.AWSClient.LaunchInstance(pubSubnet.ID, imageID, 1, "t3.medium", consts.InstanceKeyName, []string{SGID}, true)
	if err != nil {
	...
	_, err = Exec_CMD(consts.AWSInstanceUser, sshKey, hostname, cmd1)
	if err != nil {
		log.LogError("login instance to run cmd %s failed %s", cmd1, err)
		return inst, "", "", err
	}
	caContent, err := Exec_CMD(consts.AWSInstanceUser, sshKey, hostname, cmd2)
	if err != nil {
...
func (vpc *VPC) CopyImageToProxy(name string) (destinationImageID string, err error) {
	...
	sourceImageID, ok := consts.ProxyImageMap[sourceRegion]
	if !ok {
