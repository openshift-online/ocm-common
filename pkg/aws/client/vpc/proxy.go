...
import (
	...
	"github.com/openshift-online/ocm-common/pkg/aws/ssh"
...
func (vpc *VPC) LaunchProxyInstance(imageID string, zone string, sshKey string) (types.Instance, string, string, error) {
	...
	_, err = ssh.Exec_CMD(CON.AWSInstanceUser, sshKey, hostname, cmd1)
	if err != nil {
		log.LogError("login instance to run cmd %s failed %s", cmd1, err)
		return inst, "", "", err
	}
	caContent, err := ssh.Exec_CMD(CON.AWSInstanceUser, sshKey, hostname, cmd2)
	if err != nil {
