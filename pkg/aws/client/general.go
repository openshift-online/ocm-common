...
func (client *AwsV2Client) ResourceDeleted(resourceID string) bool {
...
func (client AwsV2Client) WaitForResourceExisting(resourceID string, timeout int) error {
	now := time.Now()
...
// WaitForResourceExisting will wait for the resource created in <timeout> seconds
func (client AwsV2Client) WaitForResourceDeleted(resourceID string, timeout int) error {
	now := time.Now()
	for now.Add(time.Duration(timeout) * time.Second).After(time.Now()) {
